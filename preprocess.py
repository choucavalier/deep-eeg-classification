...
def preprocess_one_file(path):
  ...
  rnd_start_idxs = np.random.randint(
    int(60 * SFREQ),
    int(x.shape[1] - (60 * SFREQ)),
    size=max(1, x.shape[1] // (60 * SFREQ) // 5)
  )
  x = np.vstack([
    x[np.newaxis, :, idx:int(idx + 60 * SFREQ)] for idx in rnd_start_idxs
  ])
  x = x.clip(-800, 800)
  y = np.repeat(int('abnormal' in path), x.shape[0])
  return (path, x, y)
